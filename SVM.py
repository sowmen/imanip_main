...
def classify(X, y, opt_params):
    ...
    scores = cross_val_score(model, X, y, cv=10, scoring='accuracy', n_jobs=-1, verbose=10)
    print(scores)
    print(np.mean(scores))
    print(np.std(scores))


def print_confusion_matrix(X_train, y_train, X_test, y_test, opt_params):
    """
    ...
    :param opt_params: The optimal hyperparameters
    """
    y_pred, y_test = get_predictions(X_train, y_train, X_test, y_test, opt_params)
...
def get_predictions(X_train, y_train, X_test, y_test, opt_params):
    """
